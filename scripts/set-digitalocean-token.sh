...
if [ -z "$DOTOKEN" ] || [ -z "$DOCLUSTERUUID" ] || [ -z "$DOTOKENNAME" ]; then
  ...
  >&2 echo "export DOTOKENNAME=<name your token here>"
  exit 1;
fi

mkdir -p "$HOME/.digitalocean"
echo "DOTOKEN=$DOTOKEN" > "$HOME/.digitalocean/$DOTOKENNAME"
echo "DOCLUSTERUUID=$DOCLUSTERUUID" >> "$HOME/.digitalocean/$DOTOKENNAME"
# Remove read access except for yourself.
chmod go-r "$HOME"/.digitalocean/tutorial-token

echo ""
echo "All done, to see your token file check:"
echo ""
echo '    cat "$HOME/.digitalocean/tutorial-token"'
echo ""
